...
✅ Siap digunakan! Kirim foto dan lokasi untuk memulai.
  ...
  async handleGeotagsStats(ctx: GeotagsContext) {
    try {
      ...
      const statsResponse = await axios.get(`${this.backendUrl}/activity/user/${ctx.user.id}/feature/geotags`);
    ...
    try {
      ...
      if (timeString.toLowerCase() === 'reset') {
        ...
        if (parsedDate) {
          ...
          await ctx.reply(`⏱️ Waktu manual diatur ke: ${parsedDate.toLocaleString('id-ID', { dateStyle: 'full', timeStyle: 'short' })}`);
        } else {
          await ctx.reply('Format tanggal/waktu tidak valid. Gunakan YYYY-MM-DD HH:MM\nContoh: /set_time 2024-01-20 10:30');
        }
      }
      ...
      if (state.alwaysTagLocation && !state.isWaitingForStickyLocation) {
        ...
        state.batchTimer = setTimeout(async () => {
          if (state.pendingPhotos && state.pendingPhotos.length > 0) {
            await this.processBatchPhotosWithGeotag(ctx, state.pendingPhotos, state.alwaysTagLocation!, state.customDateTime);
            state.pendingPhotos = [];
            delete state.batchTimer;
          }
        }, 2000);
        
        await ctx.reply(`✔️ Foto ${state.pendingPhotos.length} diterima. Batch processing dalam 2 detik...`);
      } else {
        state.pendingPhotoFileId = photoFileId;
        if (state.isWaitingForStickyLocation) {
          await ctx.reply('✔️ Foto diterima. Bot sedang menunggu Anda mengirimkan lokasi untuk diatur sebagai default AlwaysTag.');
        } else {
  ...
  async handleLocation(ctx: GeotagsContext) {
    try {
      ...
      if (state.isWaitingForStickyLocation) {
        ...
        delete state.isWaitingForStickyLocation;
        await ctx.reply(`📍 Lokasi telah diatur untuk mode AlwaysTag: ${receivedLocation.latitude.toFixed(5)}, ${receivedLocation.longitude.toFixed(5)}. Foto berikutnya akan menggunakan lokasi ini.`);
        
        if (state.pendingPhotoFileId) {
          const photoFileId = state.pendingPhotoFileId;
          delete state.pendingPhotoFileId;
          await this.processPhotoWithGeotag(ctx, photoFileId, receivedLocation, state.customDateTime);
        }
      } else if (state.alwaysTagLocation) {
        state.alwaysTagLocation = receivedLocation;
        await ctx.reply(`📍 Lokasi AlwaysTag diperbarui: ${receivedLocation.latitude.toFixed(5)}, ${receivedLocation.longitude.toFixed(5)}.`);
        ...
        if (state.pendingPhotoFileId) {
          ...
          await ctx.reply('📌 Lokasi diterima. Mohon kirim foto terlebih dahulu untuk mode standar atau aktifkan /alwaystag.');
        }
      }
  ...
  private splitAddressIntoLines(address: string, maxCharsPerLine: number, maxLines: number): string[] {
    const lines: string[] = [];
  ...
  private async generateGeotagImage(
    ...
    const maxCharsPerLine = Math.floor((availableWidth * 0.95) / (optimalFontSize * charWidthRatio));
    ...
    if (addressText.length <= maxCharsPerLine) {
      ...
      let calculatedFontSize = maxWidth / (addressText.length * charWidthRatio);
    ...
    let currentAddressY = textPadding + addressFontSize;
    
    addressLines.forEach((line, index) => {
      const yPosition = currentAddressY + (index * (addressFontSize + lineSpacing));
      svgTextElements.push(`<text x="${addressCenterX}" y="${yPosition}" class="address" text-anchor="middle">${this.escapeXml(line)}</text>`);
    });
    ...
    for (let i = 0; i <= 3; i++) {
      ...
      svgTextElements.push(`<line x1="${textPadding}" y1="${y}" x2="${textSectionWidth - textPadding}" y2="${y}" class="table-line"/>`);
    }
    
    // Vertical lines (4 lines for 3 columns)
    for (let i = 0; i <= 3; i++) {
      const x = textPadding + (i * colWidth);
      svgTextElements.push(`<line x1="${x}" y1="${tableStartY}" x2="${x}" y2="${tableStartY + coordTableHeight}" class="table-line"/>`);
    }
    ...
    svgTextElements.push(`<text x="${col2CenterX}" y="${row1Y}" class="header" text-anchor="middle">Decimal</text>`);
    svgTextElements.push(`<text x="${col3CenterX}" y="${row1Y}" class="header" text-anchor="middle">DMS</text>`);
    
    // Row 2: Latitude
    const row2Y = tableStartY + rowHeight + (rowHeight / 2) + (coordValueFontSize / 3);
    svgTextElements.push(`<text x="${col1CenterX}" y="${row2Y}" class="coord-label" text-anchor="middle">Latitude</text>`);
    svgTextElements.push(`<text x="${col2CenterX}" y="${row2Y}" class="coord-value" text-anchor="middle">${this.escapeXml(latitude.toFixed(6))}</text>`);
    svgTextElements.push(`<text x="${col3CenterX}" y="${row2Y}" class="coord-value" text-anchor="middle">${this.escapeXml(latDms)}</text>`);
    
    // Row 3: Longitude
    const row3Y = tableStartY + (2 * rowHeight) + (rowHeight / 2) + (coordValueFontSize / 3);
    svgTextElements.push(`<text x="${col1CenterX}" y="${row3Y}" class="coord-label" text-anchor="middle">Longitude</text>`);
    svgTextElements.push(`<text x="${col2CenterX}" y="${row3Y}" class="coord-value" text-anchor="middle">${this.escapeXml(longitude.toFixed(6))}</text>`);
    svgTextElements.push(`<text x="${col3CenterX}" y="${row3Y}" class="coord-value" text-anchor="middle">${this.escapeXml(lonDms)}</text>`);
    ...
    svgTextElements.push(`<text x="${datetimeX}" y="${datetimeY}" class="datetime" text-anchor="middle">${this.escapeXml(dateTimeString)}</text>`);
  ...
  private async processBatchPhotosWithGeotag(
    ...
    try {
      processingMessage = await ctx.reply(`⏳ Memproses ${photoFileIds.length} foto... Mohon tunggu sebentar.`);
      ...
      for (let i = 0; i < photoFileIds.length; i++) {
        ...
        try {
          ...
          const geotagBuffer = await this.generateGeotagImage(location.latitude, location.longitude, customDateTime);
  ...
  private async getImageFileForGeotags(fileId: string, filename: string, ctx: GeotagsContext): Promise<string> {
    try {
      const telegramId = ctx.from!.id.toString();
      ...
      this.logger.info('Direct downloaded file stats for geotags', { fullPath, size: downloadedStats.size });
