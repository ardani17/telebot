...
export class OcrHandler {
  ...
  async handleOcrStats(ctx: OcrContext) {
    try {
      ...
      const statsResponse = await axios.get(`${this.backendUrl}/activity/user/${ctx.user.id}/feature/ocr`);
    ...
    try {
      ...
      try {
        ...
        const ocrResponse = await axios.post(
          `${this.backendUrl}/files/ocr/process`,
          formData,
          {
            headers: {
              ...formData.getHeaders(),
  ...
  private async processDocumentForOcr(ctx: OcrContext, document: any) {
    ...
    try {
      ...
      if (await this.localFileService.isLocalModeAvailable()) {
        this.logger.info('Using local file copy mode for document', { fileId, filePath: fileInfo.file_path });
    ...
    const dmsPatterns = [
     ...
     for (const pattern of latLonPatterns) {
       ...
       for (const match of matches) {
         ...
         if (match[1] && match[2] && match[3] && match[4] && (match[2] === 'S' || match[2] === 'N')) {
           ...
           this.logger.info('Processing degree format coordinates', {
             originalLat: match[1], latDir: match[2],
             originalLon: match[3], lonDir: match[4],
         ...
         if (lat >= -90 && lat <= 90 && lon >= -180 && lon <= 180) {
           ...
           const isRealistic = (
             ...
             !(lat > 0 && lat < 100 && lon > 0 && lon < 100 && Math.floor(lat) === lat && Math.floor(lon) === lon) // Avoid integer temperature-like values
           );
       ...
       for (const pattern of decimalPatterns) {
         ...
         for (const match of matches) {
           ...
           if (lat >= -90 && lat <= 90 && lon >= -180 && lon <= 180) {
             ...
             const isRealistic = (
               Math.abs(lat) > 0.001 && Math.abs(lon) > 0.001 && // Not too close to 0,0
               Math.abs(lat) > 1 && Math.abs(lon) > 1 && // Must be > 1 for general patterns to avoid instrument readings
        ...
        for (const match of matches) {
          try {
            const lat = this.parseDMSToDecimal(
              parseInt(match[1]), parseInt(match[2]), parseFloat(match[3]), match[4]
            );
            const lon = this.parseDMSToDecimal(
              parseInt(match[5]), parseInt(match[6]), parseFloat(match[7]), match[8]
            );
   ...
   */
  private parseDMSToDecimal(degrees: number, minutes: number, seconds: number, direction: string): number {
