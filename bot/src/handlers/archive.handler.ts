...
export class ArchiveHandler {
  ...
  constructor(bot: Telegraf, logger: winston.Logger) {
    ...
    this.uploadDir = '/tmp/archive-uploads';
  ...
  async handleFindCommand(ctx: ArchiveContext, searchPattern?: string) {
    try {
      ...
      if (
        !archiveState ||
        archiveState.mode !== 'search' ||
        !archiveState.extractedDirs ||
        archiveState.extractedDirs.length === 0
      ) {
        await ctx.reply(
          'Anda harus menggunakan `/search` dan mengirim file arsip terlebih dahulu.'
        );
        return;
      }
      ...
      if (!searchPattern) {
        await ctx.reply(
          'Format pencarian tidak valid. Gunakan: `/find <pola>`\nContoh: `/find *.pdf` atau `/find dokumen`'
        );
        return;
      }
      ...
      for (const filePath of allExtractedFiles) {
        ...
        const baseDir =
          archiveState.extractedDirs.find(d => filePath.startsWith(d)) ||
          archiveState.extractedDirs[0];
        ...
        if (searchPattern.includes('*')) {
          ...
          isMatch =
            fileName.toLowerCase().includes(searchPattern.toLowerCase()) ||
    ...
    try {
      ...
      if (!archiveState || !archiveState.mode || archiveState.mode === 'search') {
        if (!archiveState || !archiveState.mode) {
          await ctx.reply(
            'Silakan gunakan `/zip`, `/extract`, atau `/search` terlebih dahulu untuk memulai proses.'
          );
        } else if (archiveState.mode === 'search') {
          await ctx.reply(
            'Mode search menggunakan auto-extract. Gunakan `/find <pola>` untuk mencari, lalu `/send_selected` untuk mengirim file yang ditemukan.'
          );
        }
        return;
      }
  ...
  async handleSendSelectedCommand(ctx: ArchiveContext) {
    try {
      ...
      if (
        ...
        archiveState.searchResults.length === 0
      ) {
        await ctx.reply(
          'Tidak ada file yang dipilih. Gunakan `/search`, kirim file arsip, lalu `/find <pola>` terlebih dahulu.'
        );
        return;
      }
  ...
  async handleDocument(ctx: ArchiveContext) {
    try {
      ...
      if (
        (archiveState.mode === 'extract' || archiveState.mode === 'search') &&
        fileExt !== '.zip' &&
        fileExt !== '.rar'
      ) {
        await ctx.reply(
          'Hanya file ZIP dan RAR yang dapat diekstrak atau dicari. Silakan kirim file dengan format yang benar.'
        );
        return;
      }
      ...
      if (archiveState.mode === 'zip') {
        responseMessage =
          `File "${document.file_name || 'tanpa nama'}" berhasil diterima. ` +
  ...
  private async processZipFiles(
    ...
    this.logger.info('ZIP file created and sent', {
      telegramId,
      fileCount: archiveState.files.length,
    });
  }

  private async processExtractFiles(
    ctx: ArchiveContext,
    telegramId: string,
    archiveState: UserArchiveState,
    userDir: string
  ...
  private async autoExtractForSearch(
    ctx: ArchiveContext,
    telegramId: string,
    archiveFile: string,
    userDir: string
  ...
  private initUserArchiveState(
    telegramId: string,
    mode: 'zip' | 'extract' | 'search' | null = null
  ): void {
