...
export class WorkbookHandler {
  ...
  async handleWorkbookStats(ctx: WorkbookContext) {
    try {
      ...
      const statsResponse = await axios.get(`${this.backendUrl}/activity/user/${ctx.user.id}/feature/workbook`);
      ...
      if (await fs.pathExists(mediaFolderPath)) {
        const folders = (await fs.readdir(mediaFolderPath))
          .filter(async file => (await fs.lstat(path.join(mediaFolderPath, file))).isDirectory());
  ...
  async handlePhoto(ctx: WorkbookContext) {
    try {
      ...
      if (state.newFolderPath === "") {
        await ctx.reply("Silakan ketik nama sheet (contoh: 'sheet1') untuk membuat sheet baru terlebih dahulu.");
        return;
      }
    ...
    while (state.processingQueue.length > 0) {
      ...
      try {
        ...
        if (processedCount % 10 === 0 || state.processingQueue.length === 0) {
          ...
          if (remaining > 0) {
            await ctx.reply(`✅ ${processedCount}/${initialQueueLength} foto selesai. Sisa: ${remaining} foto.`);
          } else {
            const successRate = errorCount > 0 ? ` (${errorCount} error)` : '';
            await ctx.reply(`🎉 Semua ${initialQueueLength} foto berhasil diproses${successRate}! Total foto di sheet "${path.basename(state.newFolderPath)}": ${state.downloadCount}`);
          }
        }
        ...
        if (processedCount % 10 === 0) {
          await ctx.reply(`⚠️ Ada ${errorCount} error dari ${processedCount} foto. Melanjutkan pemrosesan...`);
        }
      }
    }
  ...
  private async processPhotoTask(telegramId: string, task: PhotoProcessingTask, ctx: WorkbookContext) {
    const state = this.getUserWorkbookState(telegramId);
    ...
    try {
      ...
      this.logger.info(`Photo processed successfully: ${task.fileName} for user ${telegramId}, size: ${stats.size} bytes`);
  ...
  private async getImageFileForWorkbook(fileId: string, targetPath: string, ctx: WorkbookContext): Promise<string> {
    try {
      ...
      this.logger.info('Direct downloaded file stats for workbook', { targetPath, size: downloadedStats.size });
  ...
  private async handleSendCommand(ctx: WorkbookContext, telegramId: string, state: UserWorkbookState, mediaFolderPath: string) {
    try {
      // Check if there are photos still in queue
      if (state.processingQueue.length > 0) {
        await ctx.reply(`Masih ada ${state.processingQueue.length} foto dalam antrian. Mohon tunggu hingga semua foto selesai diproses sebelum membuat Excel.`);
        return;
      }
      ...
      if (folders.length === 0) {
        await ctx.reply("Tidak ada sheet dengan foto yang tersedia. Silakan tambahkan foto ke sheet terlebih dahulu.");
        return;
      }
      ...
      const response = await axios.post(`${this.backendUrl}/files/workbook/generate`, {
        telegramId,
        userId: ctx.user!.id,
        mediaFolderPath,
        folders: folders
